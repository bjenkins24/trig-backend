...
class GenericExtractionTest extends TestCase
{
    ...
    public function testGetWebsite(): void
    {
        ...
        $this->mock(WebsiteExtractionHelper::class, function ($mock) use ($url, $mockWebsite, $mockHtml) {
            $mock->shouldReceive('fullFetch')->with($url, 15000)->andReturn($mockWebsite);
            $mock->shouldReceive('parseHtml')->with($mockHtml, $url)->andReturn($this->getMockParseHtml($mockWebsite));
        });
    ...
    public function testGetWebsiteOnRetry(): void
    {
        ...
        $this->mock(WebsiteExtractionHelper::class, function ($mock) use ($content, $mockWebsite, $url) {
            $mock->shouldReceive('fullFetch')->andReturn($mockWebsite);
            $mock->shouldReceive('parseHtml')->with($content, $url)->andReturn($this->getMockParseHtml($mockWebsite));
        });
